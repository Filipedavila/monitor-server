...
export class EvaluationConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  private async loop() {
    ...
    while (this.running) {
      ...
      const bufferEntries: BufferEvalution = entries.reduce(
        (acc: BufferEvalution, [id, fields]: [string, string]) => {
      ...
      const now = Date.now();
      if (
        buffer.redis_ids.length >= 1000 ||
        (buffer.redis_ids.length > 0 && now - lastFlush >= this.flushInterval)
      ) {
