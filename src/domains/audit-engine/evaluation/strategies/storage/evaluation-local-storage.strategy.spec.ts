...
describe('EvaluationStorageService', () => {
  ...
  describe('cleanHtml', () => {
    ...
    it('strips scripts and styles from the head', () => {
      const html =
        '<html><head><script>evil()</script><style>.a{}</style><title>T</title></head><body><script>keepme()</script></body></html>';
  ...
  describe('saveEvaluation', () => {
    ...
    it('sanitizes the html, creates the target directory and writes all four artifact files', async () => {
      ...
      expect(createWriteStream).toHaveBeenCalledWith(join(targetDir, `${baseFileName}.html.gz.gz`));
