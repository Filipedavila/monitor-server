import { ForbiddenException, Injectable, InternalServerErrorException, NotFoundException, BadRequestException, Inject } from "@nestjs/common";
...
export class EvaluationService {
  ...
  public async evaluateManyPages(
    ...
    this.logger.log(`Evaluating ${request.pagesIds.length} pages for website ID: ${request.websiteId}`);
    ...
    if (pages.length !== request.pagesIds.length) {
      throw new BadRequestException("One or more provided pages do not exist or do not belong to the specified website.");
    }
    ...
    const targetQueue = securityContext.user.role_slug === RoleSlug.ADMIN 
  ...
  public async getEvaluationResultJson(
    ...
    const evaluation = await this.getEvaluationById(websiteId, pageId, evaluationId, securityContext);
