...
> {
  ...
  public async getManyEvaluations(websiteId: number, pageId: number, queryArgs: EvaluationQueryRequest ): Promise<{ data: Evaluation[]; count: number }> {
        const query = this.ormRepo.createQueryBuilder(`${this.alias}`);
        query.innerJoin(`${this.alias}.page`, "p")
  ...
  async updateEntityFromRawData(evaluationId:number, evaluationData: EvaluationScoring): Promise<Evaluation> {
  ...
  async findEvaluationById(websiteId: number, pageId:number,evaluationId: number, securityContext: SecurityContext): Promise<Evaluation | null> {
    const query = this.getBaseQuery()
      ...
      .andWhere(`${this.contextAlias}.contextId IN (:...contextIds)`, { contextIds: [securityContext.user.context.id] });
