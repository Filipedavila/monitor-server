...
export class Team implements IdentifiableModel, Auditable {
  ...
  @VirtualColumn({
    query: (alias) =>
      `SELECT COUNT(tu.user_id)::int FROM team_member  tu WHERE tu.team_id = ${alias}.id`,
  })
  usersCount: number = 0;

  @VirtualColumn({
    query: (alias) =>
      `SELECT COUNT(tw.website_id)::int FROM team_websites tw WHERE tw.team_id = ${alias}.id`,
  })
  websitesCount: number = 0;
}
