...
export function getContextIdByCode(code: ContextEnum): number | undefined {
  return ContextMap[code];
}

export function getContextIdByRole(role: RoleSlug): number | undefined {
  const contextCode = ContextMapByRole[role];
  return contextCode ? getContextIdByCode(contextCode) : undefined;
}
