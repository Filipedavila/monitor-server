...
export class OrganizationService {
  constructor(
    ...
    private readonly entityRepository: Repository<Organization>,
    @InjectDataSource()
  ...
  async findByShortName(shortName: string): Promise<any> {
    return this.entityRepository.findOne({ where: { shortName: shortName } });
  }

  async findByLongName(longName: string): Promise<any> {
    return this.entityRepository.findOne({ where: { longName: longName } });
  }
  ...
  async createOne(entity: Organization, websites: string[]): Promise<boolean> {
    const queryRunner = this.connection.createQueryRunner();
  ...
  async delete(entityId: number): Promise<any> {
    ...
    try {
      ...
      await queryRunner.manager.delete(Organization, { id: entityId });
