...
export class DirectoryService {
  ...
  async createOne(createDto: CreateDirectory): Promise<Directory> {
    return this.repo.createWithTags(createDto);
  }

  async update(
    updateDto: UpdateDirectory
  ): Promise<Directory> {
    return this.repo.updateWithTags(updateDto);
  }
  ...
  async getDirectory(directoryId: number): Promise<Directory> {
    return this.repo.findByIdOrFail(directoryId);
  }
}