...
export class TagService  extends BaseService {
  ...
  async findByOfficialTagName(tagName: string): Promise<Tag | null> {
    const tag = await this.tagRepository.findOne({
      where: { name: tagName, createdBy: IsNull() },
    });
    return tag;
  }
  ...
  */
  async findAllOfficial(): Promise<any> {
    return this.tagRepository.find({ where: { createdBy: IsNull() } });
  }
  ...
  async adminCount(search: string): Promise<any> {
    const count = await this.tagRepository.query(
      ...
      FROM Tag as t
      WHERE t.createdBy IS NULL AND t.name LIKE ?`,
  ...
  async createDirectory(
    ...
    try {
      ...
      for (const website of websites || []) {
        await queryRunner.manager.query(
          `INSERT INTO TagWebsite (TagId, WebsiteId) VALUES (?, ?)`,
          [insertTag.id, website.WebsiteId],
        );
      }
      ...
      if (type === "official" || type === "user") {
        ...
        if (type === "official") {
          ...
          for (const website of websites || []) {
            ...
            newWebsite.title = website.title;
            newWebsite.baseUrl = website.baseUrl;
            newWebsite.createdBy = tag.createdBy;
            newWebsite.createdAt = tag.createdAt;
  ...
  async update(
    ...
    let hasError = false;
    try {
      await queryRunner.manager.update(Tag, { id: tagId }, { name: name });
