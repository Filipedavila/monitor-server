...
export class PageService {
  ...
  async getPagesWithMetrics(websiteId: number): Promise<any> {
  ...
  async findAllFromMyMonitorUserWebsite(
    ...
    const website = await this.websiteRepository.findOne({
      where: { createdBy: userId, title: websiteName },
    });
    if (!website) {
  ...
  async findPageFromUrl(url: string): Promise<any> {
    return this.pageRepository.findOne({ where: { url: url } });
  }
    ...
    try {
      ...
      if (evalList.length === 0) {
        await queryRunner.manager.query(
          ...
          [page.id, userId, page.url, showTo, new Date(), studyUserId],
        );
      } else {
        await queryRunner.manager.query(
  ...
  async addPages(
    ...
    return !hasError;
    */
    // TODO , Total new implementation needed
    return false;
  }
  ...
  convertStringToStringArray(numStr: string): string[] {
    ...
    return numStr
      .split(",")
      .map((s) => s.substring(1, numStr.length - 1).replace(/"+/g, ""));
  }
        ...
        if (page) {
          ...
          await queryRunner.manager.query(
            ...
            WHERE 
              ...
              w.UserId = ? AND
              w.startingUrl = ?`,
            [insertPage.id, website, userId, startingUrl]
          );
        ...
        if (pageExists) {
          const websitePage = await queryRunner.manager.query(
            ...
            WHERE 
              ...
              wp.WebsiteId = w.WebsiteId AND
              wp.PageId = ?`,
            [tag, userId, website, userId, pageExists.id]
          );
  ...
  async delete(pages: number[]): Promise<any> {
    ...
    return !hasError;
        */
    throw new Error("Not implemented yet");
    // TODO   TOTAL new implementation logic needed with CASL and ACCESS TABLE
  }
