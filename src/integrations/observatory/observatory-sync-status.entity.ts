...
@Entity("observatory_sync_status")
export class ObservatorySyncStatus extends BaseModel {
  @Column({
    type: "enum",
    enum: SyncStatus,
  ...
  get progressPercentage(): number {
    ...
    return Math.round(
      (this.processedDirectories / this.totalDirectories) * 100,
    );
  }

  // Calculated field for estimated time remaining
  get estimatedTimeRemaining(): number | null {
    if (this.status !== SyncStatus.RUNNING || this.processedDirectories === 0)
      return null;
    ...
    const remainingDirectories =
      this.totalDirectories - this.processedDirectories;
