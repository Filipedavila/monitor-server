...
export class ObservatoryService {
  ...
  private loadConfig(): ObservatoryConfig {
    return {
      ...
      maxMemoryUsageMB: parseInt(
        process.env.OBSERVATORY_MAX_MEMORY_MB || "1024",
      ),
    };
  }
  ...
  async generateData(manual = false): Promise<any> {
    ...
    console.log({
      ...
      intParse: parseInt(process.env.AMSID || "-1"),
    });

    if (
      process.env.NAMESPACE === undefined ||
      parseInt(process.env.AMSID || "-1") === 0 ||
      manual
    ) {
  ...
  async generateDataChunked(manual = false, chunkSize = 20): Promise<any> {
    ...
    try {
      console.log(
        `Starting chunked data generation with chunk size: ${chunkSize}`,
      );

      // Get all directory IDs first
      const directoryIds = await this.getDirectoryIds();
      console.log(
        `Processing ${directoryIds.length} directories in chunks of ${chunkSize}`,
      );
      ...
      await this.validateChunkProcessing(
        directoryIds,
        allDirectories,
        failedChunks,
      );
      ...
      console.log(
        `Building global statistics from ${allDirectories.length} directories and ${allData.length} records...`,
      );
      ...
      console.log(
        `Final summary: ${allDirectories.length} directories, ${allData.length} records, ${failedChunks.length} failed chunks`,
      );
      ...
      if (syncStatus) {
        await this.markSyncFailed(syncStatus.id, err.message);
      }
  ...
  private async processChunkWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        console.log(
          `Processing chunk ${chunkNumber}/${totalChunks} (attempt ${attempt}/${maxRetries})`,
        );
        console.log(
          `Chunk ${chunkNumber} contains directories: [${chunk.join(", ")}]`,
        );
        ...
        console.log(
          `Chunk ${chunkNumber} query completed in ${queryTime}ms, returned ${chunkData.length} records`,
        );
        ...
        if (chunkData.length === 0) {
          ...
          if (directoryExists) {
            throw new Error(
              `Chunk ${chunkNumber} returned 0 records but directories exist`,
            );
          }
        }
        ...
        console.log(
          `Chunk ${chunkNumber} processing completed in ${processTime}ms, created ${chunkDirectories.length} directories`,
        );

        // Validate that we got directories for the chunk
        if (chunkDirectories.length === 0 && chunkData.length > 0) {
          throw new Error(
            `Chunk ${chunkNumber} failed to create directory objects from ${chunkData.length} records`,
          );
        }

        // STACK OVERFLOW PROTECTION: Test safe operations without spread operator
        console.log(
          `Testing stack safety before adding ${chunkDirectories.length} directories to results...`,
        );
        ...
        try {
          ...
          throw new Error(
            `Stack overflow or data validation error: ${err.message}`,
          );
        }

        // CRITICAL FIX: Only add results to main arrays after ALL operations succeed
        console.log(
          `Adding ${chunkDirectories.length} directories to main array (current: ${allDirectories.length})`,
        );
        ...
        console.log(
          `Adding ${chunkData.length} data records to main array (current: ${allData.length})`,
        );
        ...
        console.log(
          `✅ Chunk ${chunkNumber} successful: ${chunkDirectories.length} directories, ${chunkData.length} records`,
        );
        console.log(
          `   Running totals: ${allDirectories.length} directories, ${allData.length} records`,
        );
        ...
        console.error(
          `   Error occurred during: ${err.stack ? "processing" : "unknown phase"}`,
        );
        ...
        if (attempt === maxRetries) {
          ...
          console.error(
            `🚨 Chunk ${chunkNumber} failed after ${maxRetries} attempts - will be excluded from final result`,
          );
          console.error(
            `   No data from this chunk was added to totals (${attempt} failed attempts)`,
          );
          return false;
        } else {
          console.log(
            `⏳ Retrying chunk ${chunkNumber} (attempt ${attempt + 1}/${maxRetries}) in 5 seconds...`,
          );
  ...
  private async validateChunkProcessing(
    ...
    const processedDirectoryIds = [
      ...new Set(processedDirectories.map((d) => d.id)),
    ];
    ...
    if (duplicateCount > 0) {
      console.warn(
        `   ⚠️  Duplicate directory objects detected: ${duplicateCount}`,
      );
      console.warn(
        `   Total directory objects: ${processedDirectories.length}`,
      );
    }
    ...
    if (failedChunks.length > 0) {
      ...
      failedChunks.forEach(({ chunkNumber, chunk, error }) => {
        console.error(
          `   Chunk ${chunkNumber}: directories [${chunk.join(", ")}] - ${error}`,
        );
      });
    }
    ...
    const missingDirectories = originalDirectoryIds.filter(
      (id) => !processedDirectoryIds.includes(id),
    );
    const unexpectedMissing = missingDirectories.filter(
      (id) => !failedDirectoryIds.has(id),
    );
    ...
    if (missingDirectories.length > 0) {
      ...
      if (unexpectedMissing.length > 0) {
        console.error(
          `🚨 Unexpectedly missing (not in failed chunks): [${unexpectedMissing.join(", ")}]`,
        );
      }
    }
    ...
    console.log(
      `📈 Success rate: ${successRate.toFixed(1)}% (${actualCount}/${expectedCount})`,
    );

    // Fail if success rate is too low (less than 95%)
    if (successRate < 95) {
      throw new Error(
        `Observatory generation failed: success rate ${successRate.toFixed(1)}% is below acceptable threshold (95%)`,
      );
    }

    if (failedChunks.length > 0 && successRate < 100) {
      console.warn(
        `⚠️  Observatory generation completed with partial success (${successRate.toFixed(1)}%)`,
      );
   ...
   */
  private async validateDirectoriesExist(
    directoryIds: number[],
  ): Promise<boolean> {
  ...
  async generateDataIncremental(manual = false, chunkSize = 20): Promise<any> {
    ...
    try {
      ...
      const unchangedData =
        await this.getUnchangedObservatoryData(changedDirectoryIds);
      ...
      const mergedDirectories = [
        ...unchangedData.directories,
        ...changedDirectories,
      ];
      const mergedData = [...unchangedData.data, ...changedData];

      const listDirectories = new ListDirectories(
        mergedData,
        mergedDirectories,
      );
      const global = await this.buildGlobalStatistics(listDirectories);
      ...
      const err = error as Error;
      console.error("Error in generateDataIncremental:", err);
