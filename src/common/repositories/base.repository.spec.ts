...
export class TestEntityRepository extends EntityRepository<
  MockEntity,
  MockFilters,
  MockSorts,
  BasePagination
> {
  ...
  protected readonly filterMap: FilterMap<MockFilters, MockEntity> = {
    ...
    status: (query, value) =>
      query.andWhere(`${this.alias}.status = :status`, { status: value }),
  };
...
describe("EntityRepository (Base Repository)", () => {
  ...
  describe("saveMany", () => {
    ...
    it("should save multiple entities in chunks", async () => {
      ...
      mockOrmRepository.save.mockImplementation(async (data) => data as any);
    ...
    it("should log error and throw if save fails", async () => {
      ...
      await expect(
        repository.saveMany([{ id: 1 } as MockEntity]),
  ...
  describe("delete", () => {
    it("should delete entity by id", async () => {
      mockOrmRepository.delete.mockResolvedValue({
        affected: 1,
      } as DeleteResult);
  ...
  describe("query", () => {
    it("should execute raw SQL query with parameters", async () => {
      ...
      // Act
      const result = await repository.rawQuery(sql, params);
  ...
  describe("applyPagination", () => {
    ...
    it("should enforce minimum limit value of 1", async () => {
      await repository.find({ pagination: { limit: 1, page: 1 } });
