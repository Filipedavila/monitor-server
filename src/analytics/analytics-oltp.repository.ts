...
import { DirectorySummary } from './api-data-v2';
...
export class AnalyticsOLTPRepository {
  constructor(
    ...
    async getDirectoriesRank(): Promise<Array<DirectorySummary>> {
      return this.entityManager.query(`
        SELECT d.id, d.name, AVG(s.score) AS score
        FROM directories d
        JOIN directory_tags dt ON d.directoryId = dt.directory_id
        JOIN website_tags wt ON dt.tag_id = wt.tag_id
        JOIN websites w ON wt.website_id = w.id
        JOIN accessibility_statements s ON w.id = s.website_id
        JOIN pages p ON w.id = p.website_id
        WHERE d.show_in_observatory = 1 
        GROUP BY d.directoryId, d.name
        ORDER BY score DESC;
      `);
    }
